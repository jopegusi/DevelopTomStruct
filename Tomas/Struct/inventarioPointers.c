...
void preenche_uma_struct_produto_constante( int id, struct Produtos *p )
{
  ...
  p->Valor=158.45;    //Valor / preço (unitário) do produto
...
//--------------------------------------------------------------------------------------
void VariableInitializing( struct Produtos *p )
{
   preenche_uma_struct_produto_constante(  333 , p );
   produto_Print( p );
   produto_Print_Sumario( p );
   produto_Print_Sumario( p );
   produto_Print_Sumario( p );
}
...
//--------------------------------------------------------------------------------------
// conta a quantidade de Produtos no ficheiro de __do_global_dtors_aux
...
int writeProductToFile ( char *filename , struct Produtos *item )
{
    ...
    // ########################################################
    struct Produtos pp, *p;
    p = &pp;
    sprintf ( p->ID, "0000000056");
    sprintf ( p->Nome, "Motor Aquatico");  // - Nome do produto
    p->CAT='T';          // Categoria da Peça (i.e., M - Motor, T - Sistema de Travagem, E - Sistema Elétrico, O - Outras)
    sprintf (p->AF , "15/01/2021");    //Ano de Fabricação (Data no formato dd/mm/aaaa)
    sprintf (p->DC , "06/02/2021");    //Ano de Fabricação (Data no formato dd/mm/aaaa)
    sprintf (p->DE , "30/03/2021");    //Data da última receção desse tipo de produto (Data no formato dd/mm/aaaa)
    p->Qtd=2;       //Quantidade (atualmente) no armazém
    p->Unidade='U';   //Unidade de medida (i.e., U - Unidade, L - Litros, P - Packages)
    p->Valor=514.50;    //Valor / preço (unitário) do produto
    sprintf (p->DS , "16/02/2021");    //Data de saída (Data no formato dd/mm/aaaa)
    //prepara nome de ficheiro com numero de ID no nome
    sprintf ( p->CT ,  "caracteristicasTecProd_%s.txt", p->ID );
    //p.CT="caracteristicas.ID.txt";   //Características Técnicas (“string” com indicação do caminho (no formato: pasta + nome do ficheiro de texto) onde se podem ler as CT especificas do producto / peça
    p->TA=1;        //Total de acessos/saídas do armazém
...
int main()
{
  ...
  // Vai ver se existe ficheiro de dados com o inventario

  // se existir , vai contar quantos produtos existem lá dentro
  // e regista essa quantidade na variavel contador de produtos

  // se o ficehiro não existe cria o ficheiro novo e vazio
  // coloca a variavel global contador de produtos a Zeros

  VariableInitializing( pProduct );
  ...
  int items = readProductsFromFile ( fileInventario );
